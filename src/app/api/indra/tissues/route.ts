...
// Récupérer les tissus où un gène est exprimé
export async function POST(request: Request) {
  try {
    ...
    console.log(`Récupération des tissus pour le gène ${gene[0]}:${gene[1]}`);
    
    // Appel à l'API discovery.indra.bio
    const response = await fetch('https://discovery.indra.bio/api/get_tissues_for_gene', {
      method: 'POST',
      headers: {
    ...
    console.log(`✅ Tissus récupérés pour ${gene[0]}:${gene[1]}:`, data);

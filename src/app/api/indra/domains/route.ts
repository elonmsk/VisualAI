...
// Récupérer les domaines protéiques associés à un gène
export async function POST(request: Request) {
  try {
    ...
    console.log(`Récupération des domaines protéiques pour le gène ${gene[0]}:${gene[1]}`);
    
    // Appel à l'API discovery.indra.bio
    const response = await fetch('https://discovery.indra.bio/api/get_domains_for_gene', {
      method: 'POST',
      headers: {
    ...
    console.log(`✅ Domaines protéiques récupérés pour ${gene[0]}:${gene[1]}:`, data);
